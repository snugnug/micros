{
  ...
  supportedSystems2 ? ["x86_64-linux" "i686-linux" "armv6l-linux"],
  ...
}:
...
in {
  ...
  dist_test = fetchClosure2 (cfg:
    pkgs.runCommand "dist" {
      inherit (cfg.system.build) dist kernel;
      config = cfg.system.build.kernel.configfile;
    } ''
      #!/bin/sh
