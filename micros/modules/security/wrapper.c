...
static int make_caps_ambient(const char *self_path) {
  ...
  default:
    fprintf(stderr,
            "BUG! Unsupported capability version 0x%x on %s. Report to NixOS "
            "bugtracker\n",
            version, self_path);
    return 1;
  }
...
int main(int argc, char **argv) {
  ...
  // This mitigates vulnerabilities caused by incorrect handling in privileged
  // code.
  if (argv[0]) {
    old_argv0 = argv[0];
    argv[0] = SOURCE_PROG;
